...
BoundManager::BoundManager(Context &context)
    : _context(context), _size(0), _allocated(0), _tableau(nullptr), _rowBoundTightener(nullptr), _consistentBounds(&_context), _firstInconsistentTightening(0, 0.0, Tightening::LB), _lowerBounds(nullptr), _upperBounds(nullptr)
{
...
void BoundManager::recordInconsistentBound(unsigned variable, double value, Tightening::BoundType type)
{
    ...
    {
        std::cerr << "INCONSTISTENT BOUND<<<<<<<<  " << variable << " .    " << value << " .  " << type << std::endl;
